...
1. Download the asset ``TW1_PAR_Editor.exe`` of that release next to the
   running exe as ``TW1_PAR_Editor.exe.new``.
...
Transcribed from TW1QuestCreator/questforge2/updater.py (PY_TOOL_DESIGN.md
...
def start_swap(exe, new, pid=None):
    ...
    fd, path = tempfile.mkstemp(prefix='pareditor_update_', suffix='.bat')
